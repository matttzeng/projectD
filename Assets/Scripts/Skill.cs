...
namespace ProjectD
{
    ...
    public class Skill : EquipmentItem
    {
        ...
        /// Weapon Effect.
        /// </summary>
        public abstract class SkillAttackEffect : ScriptableObject
        {
...
public class SkillEditor : Editor
{
    ...
    List<string> m_AvailableSkillAttackEffectType;
    SerializedProperty m_SkillAttackEffectListProperty;
    ...
    void OnEnable()
    {
        ...
        m_SkillStatProperty = serializedObject.FindProperty(nameof(Skill.Stats));

        m_HitSoundProps = serializedObject.FindProperty(nameof(Skill.HitSounds));
        m_SwingSoundProps = serializedObject.FindProperty(nameof(Skill.SwingSounds));
        ...
        lookup = typeof(Skill.SkillAttackEffect);
        m_AvailableSkillAttackEffectType = System.AppDomain.CurrentDomain.GetAssemblies()
            .SelectMany(assembly => assembly.GetTypes())
            .Where(x => x.IsClass && !x.IsAbstract && x.IsSubclassOf(lookup))
            .Select(type => type.Name)
            .ToList();
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.LabelField("Skill Stats", EditorStyles.boldLabel);
        ...
        //attack
        choice = EditorGUILayout.Popup("Add new Skill Attack Effect", -1, m_AvailableSkillAttackEffectType.ToArray());

        if (choice != -1)
        {
            var newInstance = ScriptableObject.CreateInstance(m_AvailableSkillAttackEffectType[choice]);
            
            AssetDatabase.AddObjectToAsset(newInstance, target);
            
            m_SkillAttackEffectListProperty.InsertArrayElementAtIndex(m_SkillAttackEffectListProperty.arraySize);
            m_SkillAttackEffectListProperty.GetArrayElementAtIndex(m_SkillAttackEffectListProperty.arraySize - 1).objectReferenceValue = newInstance;
        }
        
        toDelete = -1;
        for (int i = 0; i < m_SkillAttackEffectListProperty.arraySize; ++i)
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.BeginVertical();
            var item = m_SkillAttackEffectListProperty.GetArrayElementAtIndex(i);           
            SerializedObject obj = new SerializedObject(item.objectReferenceValue);
        ...
        if (toDelete != -1)
        {
            var item = m_SkillAttackEffectListProperty.GetArrayElementAtIndex(toDelete).objectReferenceValue;
            DestroyImmediate(item, true);
            
            //need to do it twice, first time just nullify the entry, second actually remove it.
            m_SkillAttackEffectListProperty.DeleteArrayElementAtIndex(toDelete);
            m_SkillAttackEffectListProperty.DeleteArrayElementAtIndex(toDelete);
        }
